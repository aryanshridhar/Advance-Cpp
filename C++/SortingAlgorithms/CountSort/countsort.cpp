...
// and then traverse the newly created vector to change back the elements in main vector
// accroding to thier number of occurrences.
// O(n)
// Space: O(n)
// Not inplace.
// Works well for positive integers only. Incas eof negative integers, we need to virtually set the
// first index after minimum value in the auxillary vector. (Not space efficient algo).
void countsort(vector<int> &input){
    int maxi = maximum(input);
    vector<int> output(maxi+1, 0); // Initializes the vector with size as maxi+1 and all initial values as 0.
