...
int main() {
  ...
  array.resize(3); // will contain only first 3 elements now (Just reduces the
                   // size of array , beyond elements are still present)
  ...
  array.shrink_to_fit(); // destroys all the beyond elemets , retaining only 3
                         // elements
  ...
  array.insert(array.begin() + 2,
               {99, 11}); // inserts 99 and 11 at index 2,3 respectively (have
                          // to specify pointer location at value)
